...
arr 代表整个二维数组，类型是 int[2][3]，指向整个数组的起始位置。它的地址是数组的起始位置，也就是 arr[0] 的地址。
对于二维数组，arr 其实是一个指向二维数组首行的指针。

*arr 实际上是 arr[0]，即数组的第一行。是一个指向 arr[0][0] 的指针，类型是 int[3]。
因此，*arr 会指向数组的第一个元素 {1, 2, 3}。因为二维数组是连续存储的，arr 和 *arr 都指向内存中的同一位置。

arr[0] 是二维数组的第一行，是一个 int[3] 数组，指向第一个元素 arr[0][0]。这个地址和 *arr 是相同的，
因为 arr[0] 是 *arr 的值。
