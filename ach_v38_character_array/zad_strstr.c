...
/*
好的，下面是关于**内存空间布局**的详细讲解，包括你提到的各个内存区域在**虚拟内存中的位置（高地址 → 低地址）**。

## 🧠 程序的内存空间布局（虚拟地址空间）

一段典型的程序在内存中的布局如下图（以32位 Linux 系统为例，64位类似，布局可能不同但原理一致）：
...
1. **字符串字面量 `"Hello, world!"`** 是一个常量字符串，编译器会将它放在**代码段或只读数据段（.rodata）**。
2. **`const char *haystack`** 是一个**指针变量**，它存储了这个字符串字面量的地址。
