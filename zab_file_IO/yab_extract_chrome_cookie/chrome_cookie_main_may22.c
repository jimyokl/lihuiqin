...
int get_chrome_aes_key(unsigned char *out_key, int *out_key_len)  // ===== 读取 Local State 文件，提取和解密 AES key =====
{
    ...
    printf("\nin %s(), Local State path: %s\n", __func__, local_state_path); //C:\Users\yan\AppData\Local\Google\Chrome\User Data\Local State
	...
	*dst = '\0';
    ...
    free(json_buf);
 /*    unsigned char *decoded_key = base64_decode(base64_key, strlen(base64_key), &base64_len); // base64 解码
    if (!decoded_key) {
	...
	printf("\n");
   int ret = decrypt_aes_key(decoded_key, base64_len, out_key, out_key_len); //解密DPAPI前缀的密钥
    free(decoded_key);
    if (!ret) {
...
int copy_file(const char *src, const char *dest)
{
    ...
    if(!in || !out){
        if (in) fclose(in);
        if (out) fclose(out);
		//DWORD err = GetLastError(); printf("Copy failed. Error code: %lu\n", err);
        return 0;
    }
    char buffer[8192];
