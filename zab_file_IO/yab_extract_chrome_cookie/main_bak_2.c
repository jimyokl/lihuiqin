...
int main() {
    ...
    snprintf(temp_path, sizeof(temp_path), "%s\\temp_chrome_cookies_copy.db", getenv("TEMP"));
    if (!copy_file(db_path, temp_path)) 
    {
        printf("Unable to copy the database. Please make sure Chrome is closed.\n");
        return 1;
    }         
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) 
    {
        ...
        printf("\ndomain: %s; name: %s; path: %s;  ", domain, name, path);
    ...
    printf("success! cookies.txt, The file has been generated.\n");
    return 0;
}
